...
forvalues i=1/4 {
	...
	tab groupsize posterior_groupsize if loancycle==`i' & groupsize>0, row matcell(x`i')
}
...
forvalues i=1/3 {
	forvalues j=1/5 {
		scalar xt`i'_`j' = x`i'[`j',1] + x`i'[`j',2] + x`i'[`j',3] + x`i'[`j',4] + x`i'[`j',5] + x`i'[`j',6] + x`i'[`j',7]
	}
}
* transition 4
forvalues i=4/4 {
	forvalues j=1/5 {
		scalar xt`i'_`j' = x`i'[`j',1] + x`i'[`j',2] + x`i'[`j',3] + x`i'[`j',4] + x`i'[`j',5] + x`i'[`j',6]
	}
}
