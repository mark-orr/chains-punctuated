...
// all cycles, all groups
areg `outcome`q'' c.schdnr##c.loancycle i.officerid lnamtmemm i.nrterms i.schddateyear i.seasondummy, absorb(groupid) vce(cluster groupid)
local i=1
local coefficients`i' "1 2 3"
matrix m`i' = r(table)
scalar n`i' = e(N)
...
// all cycles, 5-cycle groups
areg `outcome`q'' c.schdnr##c.loancycle i.officerid lnamtmemm i.nrterms i.schddateyear i.seasondummy if total_cycles==5, absorb(groupid) vce(cluster groupid)
local i=2
local coefficients`i' "1 2 3"
matrix m`i' = r(table)
scalar n`i' = e(N)
